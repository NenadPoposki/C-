using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Task_2.Classes
{
    public class PetStore<T> where T : Pet
    {
        public static List<T> Pet { get; set; }
        static PetStore()
        {
            Pet = new List<T>();
        }

        public static void PrintPets(Cat cat1)
        {
            foreach (T pets in Pet)
            {
                Console.WriteLine($"Name: {pets.Name} Type: {pets.Type} Age: {pets.Age}");
            }
        }

        public static void BuyPet (T pets)
        {
            T existingItem = Pet.FirstOrDefault(x => x.Name == pets.Name);
            if (existingItem == null)
            {
                Console.WriteLine("There is no such pet with that name");
                return;
            }

            Pet.Remove(pets);
            //Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine($"The purchase was successfully completed for {existingItem.Name}");
            Console.ResetColor();

        }

        
    }
}
